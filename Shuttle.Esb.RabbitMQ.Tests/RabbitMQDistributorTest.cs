...
namespace Shuttle.Esb.RabbitMQ.Tests
{
	public class RabbitMQDistributorTest : DistributorFixture
	{
		...
		{
			TestDistributor(RabbitMQFixture.GetComponentContainer(), RabbitMQFixture.GetComponentContainer(), @"rabbitmq://shuttle:shuttle!@localhost/{0}", isTransactionalEndpoint);
		}
	}
}